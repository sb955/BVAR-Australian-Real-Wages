...
$$
p(A, \Sigma|Y,X, \Omega) \sim \operatorname{MNIW}(\bar{A}, \bar{\nu}, \bar{s}, \bar{v}).
$$
...
S = 100
# Estimate initial parameterswdw
...
for (s in 1:S) {
  ...
  if (s>=2){
    ...
    log_likelihood_star <- sum(dgamma(lambda.posterior.draws[,s], shape = lambda.nu.star/2, rate = lambda.nu.star/2, log = TRUE))
    log_prior_star <- dnorm(lambda.nu.star, mean = lambda.nu[s-1], sd = var.lambda.nu, log = TRUE)
    
    log_likelihood_curr <- sum(dgamma(lambda.posterior.draws[,s], shape = lambda.nu.star/2, rate = lambda.nu.star/2, log = TRUE))
