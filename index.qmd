...
```
One can observe, that the means represent exactly the expected values, whereas the variances lie a bit further apart from 1. Lambda 


####Include $\nu$ sampling

```{r echo=FALSE, reults='hide'}
#code used from Chan(Large Bayesian Vars)
#new prior
lambda.nu.prior = 3
...
for(s in 1:S){
  ...
  if (nuc > 2 && nuc < nuub) {
    alp_MH <- exp(funct.nu(nuc) - funct.nu(nu)) * dnorm(nu, mean = nut, sd = sqrtDnu) / dnorm(nuc, mean = nut, sd = sqrtDnu)
    if (alp_MH > runif(1)) {
      nu.draws[s] <- nuc
      binary <- 1
    }
...
Mean of $\nu$ settles around 4. Results vary a lot when adjusting the prior of $\nu$. 
...
Sammpling $/nu$ can also be of further interest, instead of using a fixed value. 
