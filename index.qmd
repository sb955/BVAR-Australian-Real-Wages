...
```{r Model set-up  , echo=TRUE, results='hide'}
...
```{r Maximum Likelihood  , echo=TRUE, results='hide'}
...
```{r Minnesota priors, echo=TRUE, results='hide'}
N = ncol(Y)
...
```{r NIW posterior paramters, echo=TRUE, results='hide'}
...
## Posterior draws using Gibbs sampler
...
```{r Simulate draws, echo=TRUE, results='hide'}
...
```{=tex}
\begin{align}
vec(U) & (0, \Sigma \otimes \Omega)
\end{align}
```

If $\Sigma=diag(\lambda_1,...,\lambda_t)$
