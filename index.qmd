...
# Get wage growth
nom.wpi <- log(subset(read_abs(series_id = "A2713851R"), date >= as.Date(start.period))$value)

# Get inflation rate (CPI)
cpi <- log(subset(read_abs(series_id = "A2325850V"), date >= as.Date(start.period))$value)
...
ppi <- log(subset(read_abs(series_id = "A2314865F"), date >= as.Date(start.period))$value)

# Get export price index (EXPI)
expi <- log(subset(read_abs(series_id = "A2294886K"), date >= as.Date(start.period))$value)

# Get import price index (IMPI)
impi <- log(subset(read_abs(series_id = "A2295765J"), date >= as.Date(start.period))$value)

# Get cost of living index (CLI)
cli <- log(subset(read_abs(series_id = "A4083524T"), date >= as.Date(start.period))$value)

#unemployment data
unemployment.data <- log(read_abs(series_id = "A84423092X"))
...
#labor participation
participation.data <- log(read_abs(series_id = "A84423093A"))
...
# Create data frame
df <- data.frame(
  "Log of Real WPI" = real.wpi,
  "Log of CPI" = cpi,
  "Log of PPI" = ppi,
  "Log of EXPI" = expi,
  "Log of IMPI" = impi,
  "Log of CLI" = cli,
  "Log of Unemployment" = unempl,
  "Log of Participation" = parti,
