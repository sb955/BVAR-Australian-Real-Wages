...
A.posterior.draws <- array(dim = c(K, N, S))
lambda.posterior.draws <- array(NA, c(t, S))
lambda.nu.draws <- numeric(S)
var.lambda.nu <- 1

# sample nu function
sample_nu2 <- function(lambda, lambda.nu, var.lambda.nu) {
  lambda.nu.star <- rnorm(1, mean = lambda.nu, sd = var.lambda.nu)
  
  if (lambda.nu.star > 2) {
    log_likelihood_star <- sum(dgamma(lambda, shape = lambda.nu.star, rate = lambda.nu.star, log = TRUE))
    log_prior_star <- dnorm(lambda.nu.star, mean = lambda.nu, sd = var.lambda.nu, log = TRUE)
    
    log_likelihood_curr <- sum(dgamma(lambda, shape = lambda.nu, rate = lambda.nu, log = TRUE))
    log_prior_curr <- dnorm(lambda.nu, mean = lambda.nu, sd = var.lambda.nu, log = TRUE)
    ...
    if (runif(1) < alpha) {
      return(lambda.nu.star)
    }
  }
  return(lambda.nu)
}

#run posterior draws with nu sampling
for (s in 1:S) {
  if (s == 1) {
    lambda.s <- lambda.0
  } else {
  ...
  for (x in 1:t) {
    ...
    lambda.posterior.draws[x, s] <- rinvgamma(1, N + lambda.nu.prior, lambda.nu.prior + u_t %*% Sigma.posterior.draws[,,s] %*% t(u_t))
  }
  
  lambda.nu.draws[s] <- sample_nu2(lambda.posterior.draws[, s], lambda.nu.prior, var.lambda.nu)
  lambda.nu.prior <- lambda.nu.draws[s]
}

# Plot the nu draws
plot(lambda.nu.draws, type = 'l', col = 'blue', lwd = 2, 
