...
stats = posterior_t(Y.bv, X.bv, 1, 100)

round(apply(stats[[1]], 1:2, mean),2)
...
A possible further extension of the model would be the implementation of a sampling algorithm. This can be applied using a Metropolis-Hastings structure. 
...
forecasts <- function(Y, A.posterior, Sigma.posterior, h, S, p){
  N = ncol(Y)
...
```{r}
forecast.table <- function(Y.h1, Y.h2, y) {
  ...
  for (x in 1:1) {
    point.f1.table <- apply(Y.h1[, x, ], 1, mean)
    point.f2.table <- apply(Y.h2[, x, ], 1, mean)
    
    for (period in 1:n_periods) {
      forecast_table <- rbind(forecast_table, data.frame(
        Period = period,
        Variable = var_names[x],
        Baseline_Forecast = point.f1.table[period],
        Extension_Forecast = point.f2.table[period]
      ))
    }
  }
...
forecast.table.out <- forecast.table(forecasts_norm$Y.h, forecasts_t$Y.h, y)
print(forecast.table.out)
```
...
forecast.table2 <- function(Y.h1, Y.h2, y) {
  ...
  for (x in 1:1) {
    point.f1.table2 <- apply(Y.h1[, x, ], 1, mean)
    point.f2.table2 <- apply(Y.h2[, x, ], 1, mean)
    
    for (period in 1:n_periods) {
      # Calculate percentage changes for logged values
      actual_value <- y[nrow(y) - n_periods + period, x]
      baseline_pct_change <- ((exp(point.f2.table2[period]) - exp(actual_value)) / exp(actual_value)) * 100
      extension_pct_change <- ((exp(point.f2.table2[period]) - exp(actual_value)) / exp(actual_value)) * 100
      
      forecast_table <- rbind(forecast_table, data.frame(
        Period = period,
        #Variable = var_names[x],
        Baseline_Forecast = point.f1.table2[period],
        Extension_Forecast = point.f2.table2[period],
        Baseline_Percentage_Change = baseline_pct_change,
        Extension_Percentage_Change = extension_pct_change
      ))
    }
  }
