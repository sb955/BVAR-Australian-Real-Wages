...
```{r Maximum Likelihood  , echo=FALSE, results='hide'}
XX = solve(t(X)%*%X)
...
```{r Minnesota priors, results='hide'}
N = ncol(Y)
...
## Define posterior paramters 

```{r NIW posterior paramters, results='hide'}
...
```{r Posterior darws ,  message=FALSE, warning=FALSE, results='hide'}
...
```{r Joint predictive density 1 period ahead, warning=FALSE}
# Index for Log.of.Real.WPI
log_wpi_index <- 1

# Variable names excluding Log.of.Real.WPI
variable_names <- c("Log.of.CPI", "Log.of.PPI", "Log.of.EXPI", "Log.of.IMPI",
                    "Log.of.CLI", "log.unempl", "log.parti", "log.h.worked",
                    "Log.of.Real.GDP")

par(mfrow=c(1, 2))  # Adjusted for 10 plots

# Number of bands for kernel density estimation
bands <- 100

# Iterate over the indices of variable_names
for (i in seq_along(variable_names)) {
    # Compute the joint kernel density estimation
    predictive.one.kernel <- kde2d(x = Y.h[1, 1, ], y = Y.h[1, i + 1, ], n = bands)
    
    # Calculate marginal densities for the plot
    marginal.x <- apply(predictive.one.kernel$z, 1, sum)  
    marginal.y <- apply(predictive.one.kernel$z, 2, sum)  
    
    # Scale marginals to fit the plot's z range
    ...
    # Plot a transparent surface first
    f1 <- persp3D(x = predictive.one.kernel$x, y = predictive.one.kernel$y, z = predictive.one.kernel$z,
                  phi = 25, theta = 25,
                  xlab = variable_names[i], ylab = "Log.of.Real.WPI", zlab = "Predictive Density",
                  shade = 0, border = NA, ticktype = "detailed", nticks = 2,
                  cex.lab = 1, col = "white")

    # Add lines for marginal densities
    # Line for the marginal density of x
    ...
    # Line for the marginal density of y
    ...
    # Plot the surface again with less transparency
    f1 <- persp3D(x = predictive.one.kernel$x, y = predictive.one.kernel$y, z = predictive.one.kernel$z,
...
```{r Forecast horizons, warning=FALSE}
par(mfrow=c(1, 2))  # Adjusted for 10 plots
...
par(mfrow=c(1, 2))
for (i in seq_along(variable_names)) {
    # Compute mean forecast and intervals for the ith variable
    point.f <- apply(Y.h[, i + 1, ], 1, mean)
    interval.f <- apply(Y.h[, i + 1, ], 1, hdi, credMass = 0.90)
    range_val <- range(y[, i + 1], interval.f)

    # Historical and forecast plot for the ith variable
    plot(1:(length(y[, i + 1]) + h), c(y[, i + 1], point.f), type = "l", ylim = range_val, axes = FALSE, xlab = "", ylab = "", lwd = 2, col = mcxs1)
    axis(1, c(3, 43, 83, 123, 163, 203, 243, nrow(y), nrow(y) + h), c("1960", "1970", "1980", "1990", "2000", "2010", "2020", "", ""), col = mcxs1)
    axis(2, c(range_val[1], mean(range_val), range_val[2]), c("", variable_names[i], ""), col = mcxs1)
    abline(v = nrow(y), col = mcxs1)  # Assumes y contains the historical data up to the last observed point

    # Polygon for the forecast interval
    polygon(c((length(y[, i + 1]) + h):length(y[, i + 1]), (length(y[, i + 1]) + h):length(y[, i + 1])),
            c(y[nrow(y), i + 1], interval.f[1, ], rev(interval.f[2, ]), y[nrow(y), i + 1]),
