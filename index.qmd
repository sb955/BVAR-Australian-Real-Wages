...
# Get wage growth
log.nom.wpi <- log(subset(read_abs(series_id = "A2713851R"), date >= as.Date(start.period))$value)

# Get inflation rate (CPI)
log.cpi <- log(ifelse(subset(read_abs(series_id = "A2325850V"), date >= as.Date(start.period))$value == 0, NA, subset(read_abs(series_id = "A2325850V"), date >= as.Date(start.period))$value))
...
log.ppi <- log(subset(read_abs(series_id = "A2314865F"), date >= as.Date(start.period))$value)

# Get export price index (EXPI)
log.expi <- log(subset(read_abs(series_id = "A2294886K"), date >= as.Date(start.period))$value)

# Get import price index (IMPI)
log.impi <- log(subset(read_abs(series_id = "A2295765J"), date >= as.Date(start.period))$value)

# Get cost of living index (CLI)
log.cli <- log(subset(read_abs(series_id = "A4083524T"), date >= as.Date(start.period))$value)
