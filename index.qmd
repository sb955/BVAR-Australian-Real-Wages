...
```{r}
posterior.sv = function(Y,X,priors,S){
...
# Applying function
posterior.draws.SV = posterior.sv(Y=Y, X=X, priors=priors, S=S)
round(apply(posterior.draws.SV$Sigma.posterior, 1:2, mean),8)
round(apply(posterior.draws.SV$A.posterior, 1:2, mean),3)
```

### StochasticV + T-distibution
...
posterior.sv.t = function(Y,X,priors,S){
  ...
  for (s in 1:sum(S)){
    ...
    #draw lambda
    for (x in 1:t){
      u_t <- Y[x,] - t(X[x,]) %*% A.posterior[,,s]
      lambda = rinvgamma(1, (N+lambda.nu.prior)/2, (lambda.nu.prior + u_t %*% Sigma.posterior[,,s] %*% t(u_t))/2 )
      lambda.posterior.draws[x,s] <- lambda
    }
...
forecasts_t <- function(Y, A.posterior.draws, Sigma.posterior.draws, lambda.posterior.draws, h, S, p){
  ...
  for (s in 1:S){
    ...
    for (i in 1:h){
      x.T         = c(1,as.vector(t(x.Ti)))
      Y.h[i,,s]   = rmvnorm(1, mean = x.T%*%A.posterior.draws[,,s], sigma= (lambda.posterior.draws[,s] %*% diag(t)) %*% Sigma.posterior.draws[,,s]) 
