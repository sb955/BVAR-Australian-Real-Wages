...
forecasts_norm <- forecasts(Y, post.results$A.posterior, post.results$Sigma.posterior, 8, 1000, 4)

post.t.results <- posterior_t(Y, X, 4, 1000)

forecasts_t <- forecasts(Y, post.t.results$A.posterior.draws, post.t.results$Sigma.posterior.draws, 8, 1000, 4)
```
...
forecast.table2 <- function(Y.h1, Y.h2, y) {
  ...
  for (x in 1:1) {
    ...
    actual_value_0 <- y[nrow(y), x]
    forecast_table <- rbind(forecast_table, data.frame(
      Period = 0,
      Baseline_Forecast = actual_value_0,
      Extension_Forecast = actual_value_0,
      Baseline_Percentage_Change = 0,
      Extension_Percentage_Change = 0
    ))
    
    for (period in 1:n_periods) {
      # Calculate percentage changes for logged values
      actual_value <- y[nrow(y) - n_periods + period, x]
      baseline_pct_change <- ((exp(point.f1.table2[period]) - exp(actual_value)) / exp(actual_value)) * 100
      ...
      forecast_table <- rbind(forecast_table, data.frame(
        ...
        Baseline_Percentage_Change = round(baseline_pct_change,2),
        Extension_Percentage_Change = round(extension_pct_change,2)
      ))
    }
  }
