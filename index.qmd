...
$$p\left(A,\Sigma|Y,X\right)&=MN\left(\bar{A},\bar{V},\bar{S},\bar{\nu}\right)\\\bar{V}=\left(X'\text{diag}\left(\sigma^{2}\right)^{-1}X+\underline{V}^{-1}\right)^{-1}$$
...
The forecast plots show deviating results in the SV model. Generelly, the predictions in the SV extensions are more positive than the baseline predictions. The SV predictions seem to capture more of the historical data trends. Adddiotionally. the SV modelshave smaller intervalls (68% level).
...
```
Forecasting real wages using a stochastic volatility model predicts slightly increasing levels in the upcoming two years.
